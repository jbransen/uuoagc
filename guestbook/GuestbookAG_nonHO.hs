...
data TGuestbook top
  = TGuestbookEmpty {
      ...
      tguestbook_v0 :: (TGuestbook top) -> ((Set Name, DL), TGuestbook top),
...
semGuestbook_Empty = TGuestbookEmpty {
  ...
  update cur = tickAG cur
  v0 :: (TGuestbook top) -> ((Set Name, DL), TGuestbook top)
  v0 cur = tickAG ((_lhsOsignedIn, _lhsOtrueReviews), res) where
    (_lhsOsignedIn, _lhsOtrueReviews) = tickAG (realv0 ())
    res = tickAG (update (cur {
                            tguestbook_v0 = memv0,
                            tguestbook_v0_dirty = False
                          }))
    memv0 :: (TGuestbook top) -> ((Set Name, DL), TGuestbook top)
    memv0 cur' = tickAG (if not (tguestbook_v0_dirty cur')
                         then ((_lhsOsignedIn, _lhsOtrueReviews), cur')
                         else v0 cur')
  realv0 :: () -> (Set Name, DL)
  realv0 () = (_lhsOsignedIn, _lhsOtrueReviews) where
...
semGuestbook_Arrive _name _tl = TGuestbookArrive {
  ...
  v0 :: (TGuestbook top) -> ((Set Name, DL), TGuestbook top)
  v0 cur = tickAG ((_lhsOsignedIn, _lhsOtrueReviews), res) where
    ((_lhsOsignedIn, _lhsOtrueReviews), tl) = tickAG (realv0 (tguestbook_tl cur))
    res = tickAG (update (cur {
                            tguestbook_v0 = memv0,
                            tguestbook_v0_dirty = False,
                            tguestbook_tl = tl
  ...
  realv0 :: (TGuestbook top) -> ((Set Name, DL), TGuestbook top)
  realv0 tl0 = ((_lhsOsignedIn, _lhsOtrueReviews), tl1) where
    ((_tlIsignedIn, _tlItrueReviews), tl1) = tguestbook_v0 tl0 tl0
...
semTop_Top _gb = TTopTop {
  ...
  v0 cur = tickAG (_lhsOaverage, res) where
    (_lhsOaverage, revs, gb) = tickAG (realv0 (ttop_gb cur))
    res = tickAG (update (cur {
                            ttop_v0 = memv0,
                            ttop_v0_dirty = False,
                            ttop_gb = gb
  ...
  realv0 :: (TGuestbook top) -> (Double, TDL DL, TGuestbook top)
  realv0 gb0 = (_lhsOaverage, revs1, gb1) where
    ((_gbIsignedIn, _gbItrueReviews), gb1) = tguestbook_v0 gb0 gb0
    _loc_inst_revs = tickSem _gbItrueReviews
    revs0 = semDL _loc_inst_revs
