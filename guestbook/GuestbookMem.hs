...
main = do
  ...
  print g
  let changes = [ ("insert", 1000, LeaveR "COX" 7.8 "It was great!" (Ref 1000))
                , ("delete", 1000, Ref 1001) ]
  let allChanges = concat $ replicate 1000 $ map (translateChanges gb) changes
  let f st ch = do
--        performGC
        let (g,st') = computeGradeI st ch
        print g
        return st'
  foldM_ f st allChanges
